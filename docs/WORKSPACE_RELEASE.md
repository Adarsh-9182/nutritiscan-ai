...
## The companion AI engine, reconnected — 18 September 2026

Before this change the live site had no engine at all. The agent in `lib/workspace/health-agent.ts` accepts a `Completion`; the only one wired up was the on-device model, which needs WebGPU and a ~1 GB download, so an ordinary visitor received reference notes and nothing written for them. `lib/agents/provider.ts` — the Gemini/Gateway ladder from the earlier product — was still in the repository and unreachable from the product.

- `lib/workspace/cloud-model.ts` supplies that missing argument. `hostedEngine()` resolves, per call, to the Gemini/Gateway ladder (`GOOGLE_GENERATIVE_AI_API_KEY`, then `AI_GATEWAY_API_KEY`/`VERCEL_OIDC_TOKEN`), then to an operator's OpenAI-compatible endpoint (`HEALTH_MODEL_*` with `HEALTH_MODEL_APPROVED=true`), then to nothing. Nothing configured remains a supported state: the companion answers from its reference notes and labels the answer accordingly.
- Hosted turns run through the same agent as the browser: escalation rules first, dose/treatment boundary, deterministic reference retrieval, allowlisted read tools, the fail-closed output check and deterministic record arithmetic. Only the explanation is generated. The prompts carry the question, the reference notes and up to two earlier user messages — not the profile, reports, observations or log entries, which are computed in and appended after generation. `lib/workspace/cloud-model.test.ts` asserts that boundary.
- The ladder steps down on failure. Free-tier quota is metered per model, so the next rung is a different meter rather than a retry against an exhausted one. A caller's cancellation is re-thrown, never retried.
- `POST /api/workspace/assistant` now returns a full agent reply (steps, sources, drafts) for signed-in accounts, replacing the single ungrounded completion. `POST /api/workspace/assistant/demo` serves the fictional demo without a session, metered at 10 turns per minute per network address by the in-memory limiter in `lib/http/guard.ts` — per instance, reset on cold start, and answering over a copy of fixed fiction.
- The browser prefers a loaded on-device model over the server, so enabling private AI still keeps the question in the tab. If a hosted call fails, the browser re-runs the agent locally and returns the reference notes rather than an empty turn. Telegram replies use the hosted engine when one is configured.
- Every answer states the engine that produced it: "NutritiScan AI", "Operator-configured model", "On-device AI" or the reference-notes label.
- This supersedes the 16 September line about free cloud inference never being enabled: it is enabled only by a credential an operator sets, its data boundary is stated above and in the privacy notice, and `GOOGLE_GENERATIVE_AI_API_KEY` is absent by default. Review the [Gemini API data-use terms](https://ai.google.dev/gemini-api/terms) — free-tier terms permit provider use of prompts to improve products — before pointing it at real questions, and prefer the on-device or a self-hosted engine where that matters.

## Public site redesign — 18 September 2026

The marketing page was rebuilt as `nl-*` components and a self-contained stylesheet section at the end of `app/globals.css`: a dark aurora hero with a sticky glass navigation, a product preview showing the agent's steps and source chip, a scrolling topic strip, a bento capability grid, a three-step explainer, and a privacy/limits pair that keeps the honest limits above the fold of the footer. The workspace, policy pages and the shared `Brand` mark keep the light application theme — every new rule is scoped under `.nl-root`. Motion respects `prefers-reduced-motion`; the layout is checked down to 360 px.
...
The account workspace model experiment is constrained to explaining the included lab-reference source. It uses bounded structured output, source IDs, a timeout and a fail-closed output check. This is not broad clinical RAG and the lexical filter is not a medical safety guarantee. Failed/unconfigured generation is explicitly labelled. The separate `/chat` interface uses browser-held context and the public, rate-limited chat route; it cannot read encrypted account records. `LEGACY_CLINICAL_ENABLED` stays false for the retired scan endpoint.
